...
public class CultureService {
    ...
    public Map<String , Double> geocoding(String addr) {
    	...
    	try {
            ...
            conn.setRequestMethod("GET");
            conn.setRequestProperty("Content-type", "application/json");
            conn.setRequestProperty("X-NCP-APIGW-API-KEY-ID", "476wf30zkc");//앱 등록 시 발급받은 Client ID *(명세서에서 잘 찾아보기)ㄴ
            ...
            long totalCount = (long) metaObject.get("totalCount");
            System.out.println(totalCount);
            
            if(totalCount ==0) {
            	return null; //데이터가 존재하지 않을 시, 메소드 종료
            }
